        ...
        def backtrack(moves: List[int], positions: List[Tuple[int, int]],
            ...
            if len(moves) == self.protein.length - 1:
                ...
                if energy < best_energy:
                    best_energy = energy
                    best_conf = conf
                    print(f"    ★ New best: Energy {energy} at node {nodes}")
                return  # Continue searching for better solutions
...
def run_benchmark(sequences: List[Tuple[int, str, int]], max_time: int = 30):
    ...
    for seq_idx, (length, seq_str, optimal_energy) in enumerate(sequences, 1):
        ...
        # 1. Improved CSP with AC-3
        print("--- Improved CSP Solver (AC-3 + Relative Moves) ---")
...
if __name__ == "__main__":
    ...
    results, timestamp = run_benchmark(sequences, max_time=300)
